...
regular_expression::regular_expression(char y, char x) {
    inf = false;
    is_1 = (y == '1');
    if (y == x) {           // if the letter is x
        has_full = true;
        full_length = 1;
        max_prefix = 1;
        max_suffix = 1;
        max_postfix = 1;
    } else {                // if the letter is not x
        has_full = false;
        full_length = 0;
        max_prefix = 0;
        max_suffix = 0;
...
regular_expression::regular_expression(regular_expression &reg1, regular_expression &reg2, char y) {
...
regular_expression::regular_expression(regular_expression &reg1, char y) : inf(reg1.inf), is_1(reg1.is_1),
has_full(reg1.has_full),
full_length(reg1.full_length),
max_prefix(reg1.max_prefix),
max_postfix(reg1.max_postfix) {
